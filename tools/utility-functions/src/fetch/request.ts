...
// This is a fetch wrapper written by myself :)
// just to remind myself that axios is not bad.
// WARNING
// this file NEEDS EDITING (like setting the url prefix)

import { queryString } from '../url/queryString';

const storageGetter = sessionStorage.getItem; // for token handling
const API_FALLBACK = 'http://localhost:8080';
const urlPrefix = String((window as any).API_URL ?? API_FALLBACK).replace(/\/+$/, '');
...
export function request(url: string, options: IRequestOptions = {}) {
  ...
  const authToken = options.token ?? storageGetter('token');
  if (authToken) {
